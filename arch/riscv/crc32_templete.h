...
#define LSB_CRC 1
#define CRC_BITS	(8 * sizeof(crc_t))
#define asm __asm__
...
static inline uint64_t le64_to_cpup(const uint64_t *p) {
  return *p;
}
...
static inline unsigned long crc_clmul_prep(crc_t crc, unsigned long msgpoly) {
#if LSB_CRC
  return msgpoly ^ crc;
#else
 ...
 */
static inline crc_t crc_clmul_long(unsigned long msgpoly,
   ...
   *
   * msb-first:
   *    floor((msgpoly * floor(x^(BITS_PER_LONG-1+n) / G)) /
   * x^(BITS_PER_LONG-1)) lsb-first: floor((msgpoly *
   * floor(x^(BITS_PER_LONG-1+n) / G) * x) / x^BITS_PER_LONG)
   *
   ...
   *
   *    product := ((G - x^n) * x^(BITS_PER_LONG - n)) * floor((msgpoly * x^n) /
   * G) * x crc := floor(product / x^(BITS_PER_LONG + 1 - n)) mod x^n
   *
...
static inline crc_t
crc_clmul_update_partial(crc_t crc, const uint8_t *p, size_t len,
                         const struct crc_clmul_consts *consts) {
...
static inline crc_t crc_clmul(crc_t crc, const void *p, size_t len,
                              const struct crc_clmul_consts *consts) {
  size_t align;
